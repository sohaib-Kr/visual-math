export function emphasize(elements, options = {}) {
    ...
    // Set up mutation observers for all elements
    const observers = [];
    ...
    trackedElements.forEach(item => {
      const observer = new MutationObserver(() => item.update());
      observer.observe(item.original.node, observerConfig);
      // Also observe parent groups up to SVG root
      let parent = item.original.node.parentNode;
    ...
    return {
      remove: () => {
        ...
        trackedElements.forEach((item,index) => item.original.attr({opacity:initialOpacity[index]}))
